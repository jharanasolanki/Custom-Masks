...
$amount=0;
$sql="SELECT * from maskscart where accountid=$accountid and status='Added to Cart';";
...
while($row = $result->fetch_assoc())
{
    ...
    $sql="update maskscart set status='ordered' where id=$cart_id;";
    $conn->query($sql);

    $sql="insert into payments(payment_amount,qty,payment_status,itemid,page,orderfrom,custid,createdtime) values($total,$qty,'Completed',$cart_id,'mask','cart',$accountid,'".date('Y-m-d H:i:s')."');";
    $conn->query($sql);

    $sql="insert into orders(custid,productid,qty,price,total,page,orderfrom,date) values($accountid,$cart_id,$qty,$price,$total,'mask','cart','".date('Y-m-d H:i:s')."');";
    $conn->query($sql);
}
