...
function preview() {
	...
	<?php } else {
		...
		$sharelink = preg_replace('|/lib|', '',$sharelink);
		$link = $sharelink.'?share='.$_SESSION['filename'];
	?>
	...
	html2canvas(document.getElementById('details')).then(function(canvas) {
		 //document.body.appendChild(canvas);
			...
			printWindow.document.write('{');
				printWindow.document.write('size: auto;   /* auto is the initial value */');
				printWindow.document.write('margin: 0mm;  /* this affects the margin in the printer settings */');
			printWindow.document.write('}');
